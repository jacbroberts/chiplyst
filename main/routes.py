...
def account():
    ...
    return render_template('account.html',title='Account', image_file=image_file, form=form, user=user, posts=posts)
...
def new_list():
    form = PostForm()
    if form.validate_on_submit():
        lists = Lists(title=form.title.data, content=form.content.data, author=current_user)
        db.session.add(lists)
        db.session.commit()
        flash('Your list has been created!','success')
        return redirect(url_for('lists'))
    return render_template('create_post.html', title='New List', form=form,
    legend='New List')
